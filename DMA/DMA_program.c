...
void MDMA_VidSetConfiguration( u8 Copy_u8Channel , u8 Copy_u8Direction , u8 Copy_u8DMAMode , u8 Copy_u8PINC ,
	...
	//Disable DMA
	CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 0 );
	...
	if( Copy_u8Direction == MEM_TO_MEM ){
		...
		CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 4  );
	...
	if( Copy_u8DMAMode == DMA_ENABLE_CIRCULAR ){
		...
		CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 5  );
	...
	if( Copy_u8PINC == PERIPHERAL_INCREMENT_ENABLE ){
		...
		CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 6  );
	...
	if( Copy_u8MINC == MEM_INCREMENT_ENABLE ){
		...
		CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 7  );

	}
	////////////////////////// MEMORY SIZE //////////////////////////////
	if( Copy_MEMSize == BYTE ){

		CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 10 );
		CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 11 );
	...
	if( Copy_u8PERSize == BYTE ){
		...
		CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 8 );
		CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 9 );
	...
	if( Copy_u8ChannelPriority == DMA_LOW ){
		...
		CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 12 );
		CLEAR_BIT( MDMA->CHANNEL[ Copy_u8Channel ].CCR , 13 );
